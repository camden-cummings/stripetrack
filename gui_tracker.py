        ...
        while not done.is_set():
            ...
            if r.mode_noblur_img is None:
                ...
                if gui.contours_updated or r.mode_updated:
                    ...
                    gui.save_rois(str(PurePath(f'{self.exp_folder}/{self.folder_name}.cells')))
                ...
                curr_time = "_".join(str(timer.formatted_time(timer.now() % 86400)).strip("[]").split(", "))
...
if __name__ == '__main__':
    ...
    exp_folder, event_schedule, folder_name = check_directory(exp_folder, event_schedule)
    
    poolrun = GUIPoolRun(exp_folder, event_schedule, folder_name, debug, frame_width, frame_height)
