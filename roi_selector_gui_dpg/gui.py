...
class GUI:
    ...
    def setup_roi_buttons(self, shift, curr_dir, curr_name, state_manager):
        """ROI Mode Buttons."""
        with dpg.group(label="roi buttons", pos=[shift, 25]) as roi:
            dpg.add_button(label="New ROI", callback=state_manager.new_roi)

            with dpg.file_dialog(directory_selector=False, show=False, callback=state_manager.roi_interface.load_rois_callback, id="roi_load_file", width=700, height=400, default_path=curr_dir, default_filename=curr_name):
            ...
            with dpg.file_dialog(directory_selector=False, show=False, callback=state_manager.roi_interface.save_rois_callback, id="roi_save_file", width=700, height=400, default_path=curr_dir, default_filename=curr_name):
                dpg.add_file_extension(".cells", color=(
                    0, 255, 0, 255), custom_text="[ROI Save File]")
            ...
            dpg.add_button(label="Clear Screen and Start Over", pos=[
                           shift, 120], callback=state_manager.clear_window)
            dpg.add_text(
    ...
    def setup_line_buttons(self, shift, state_manager):
        ...
        with dpg.group(label="line buttons", pos=[shift, 25]) as line:
            ...
            dpg.add_input_text(width=15, source="int_value", default_value=1, pos=[
                shift+104, 25], callback=state_manager.line_interface.num_of_vert_lines_changer)
            dpg.add_button(label="Horizontal Line",
                           callback=state_manager.line_interface.horizontal_line_callback)
            dpg.add_input_text(width=15, source="int_value", default_value=1, pos=[
                shift+118, 48], callback=state_manager.line_interface.num_of_hor_lines_changer)
            ...
            dpg.add_button(label="Clear Screen and Start Over", pos=[
                           shift, 145], callback=state_manager.clear_window)
            dpg.add_text(
            ...
            with dpg.child_window(border=False):
                with dpg.group() as roi_and_line_selection:
                    ...
                    roi = self.setup_roi_buttons(
                        shift, curr_dir, curr_name, state_manager)
                    line = self.setup_line_buttons(shift, state_manager)
                    dpg.hide_item(line)

                post_line = self.setup_post_line_buttons(
                    shift, state_manager, frame_width, frame_height, curr_dir, curr_name)
                dpg.hide_item(post_line)

            dpg.add_image("texture_tag", pos=[8, 8])

        dpg.set_primary_window(window, True)
        dpg.create_viewport(width=int(frame_width+275),
                            height=frame_height+20, title="ROI Selector")
        dpg.setup_dearpygui()
        dpg.show_viewport()
